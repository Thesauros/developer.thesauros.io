  ...
  },
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        description: 'Current Particle Swarm Optimization allocation: per-vault weights maximizing risk-adjusted return under a diversification cap.',
        params: [{ name: 'asset', in: 'query', type: 'enum', required: false, options: ['USDC', 'USDT'], desc: 'Restrict to one asset', example: '' }],
        returns: '200 \u2014 PsoAllocation',
      },
      {
        id: 'analytics-backtest',
        method: 'GET',
        path: '/analytics/backtest',
        summary: 'Run a backtest',
        description: 'Replay the historical rate series through one strategy. Returns equity curve, APY, volatility, drawdown.',
        params: [
          { name: 'strategy', in: 'query', type: 'enum', required: false, options: ['aave-only', 'best-apy', 'risk-adjusted-pso'], desc: 'Allocation strategy', example: 'risk-adjusted-pso' },
          { name: 'asset', in: 'query', type: 'enum', required: false, options: ['USDC', 'USDT'], desc: 'Filter by asset', example: '' },
          { name: 'from', in: 'query', type: 'string', required: false, desc: 'Start (epoch ms or YYYY-MM-DD)', example: '2026-05-01' },
          { name: 'to', in: 'query', type: 'string', required: false, desc: 'End (epoch ms or YYYY-MM-DD)', example: '2026-07-30' },
        ],
        returns: '200 \u2014 Backtest',
      },
      {
        ...
        description: 'Run all strategies over the same range and compare final value, APY, volatility and uplift vs the aave-only baseline.',
        params: [
          { name: 'asset', in: 'query', type: 'enum', required: false, options: ['USDC', 'USDT'], desc: 'Filter by asset', example: 'USDC' },
          { name: 'from', in: 'query', type: 'string', required: false, desc: 'Start (epoch ms or YYYY-MM-DD)', example: '' },
          { name: 'to', in: 'query', type: 'string', required: false, desc: 'End (epoch ms or YYYY-MM-DD)', example: '' },
        ],
        returns: '200 \u2014 BacktestComparison',
      },
    ],
  },
];
